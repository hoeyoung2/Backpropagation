...
double O2[HiddenUnitNo];                                    // 은익층 노드 출력값 
double O3[OutputUnitNo];                                    // 출력층 노드 출력값
double t[MaxPatternNo][OutputUnitNo];                       // 교사신호
double W21[HiddenUnitNo][InputUnitNo];                      // 가중치 (입력층 -> 은익층)
double dW21[HiddenUnitNo][InputUnitNo];                     // 가중치 (입력층 -> 은익층) Feed Back
double W32[OutputUnitNo][HiddenUnitNo];                     // 가중치 (은익층 -> 출력층)
double dW32[OutputUnitNo][HiddenUnitNo];                    // 가중치 (은익층 -> 출력층) Feed Back
double bias2[HiddenUnitNo];                                 // 은익층 노드 활성화
double dbias2[HiddenUnitNo];                                // 은익층 역방향 노드 활성화
...
void main(int argc, char* argv[])
{
    ...
    {
        for (j = 0; j < learning_pattern_no; j++){                      // 학습 데이터 수
            propagation(j);                                             // 순방향 학습
            back_propagation(j);                                        // 역방향 학습
        }
        for (errorfunc = 0.0, j = 0; j < learning_pattern_no; j++){        // 학습 데이터 수
            printf("%d", ++i);
            state(j);                                                   // state() 함수
            for (k = 0; k < OutputUnitNo; k++)                         // 출력 노드 수 
             errorfunc += pow(t[j][k] - O3[k], 2.0);                 // 목표값 - 출력값에 의한 Error 값 산출
        }
        ++num;
        errorfunc /= 2;
        printf("ErrorFunc : %.3f\n", errorfunc);                        // Error 값 모니터에 출력
        fprintf(fp, "%d : %lf \n", num, errorfunc);                      // Error값 측정 반복 및 값 저장
    }
    fclose(fp);                                                         // 파일 쓰기 닫기
    ...
    /* 신규 데이터 예측 결과 */
    for (i = learning_pattern_no; i < learning_pattern_no + test_pattern_no; i++)        // 신규 데이터 수 만큼 예측
        state(i);
}
...
void back_propagation(p){
    int i, j;
    double d2[HiddenUnitNo];                                            // 가중치 변경 입력 -> 은익
    double d3[OutputUnitNo];                                            // 가중치 변경 은익 -> 출력
...
/* 평균제곱오차: dj=(oj-yj) f'(i,j) */
    ...
    for (i = 0; i < HiddenUnitNo; i++)
    {
        for (sum = 0.0, j = 0; j < OutputUnitNo; j++)
        {
            ...
            sum += d3[j] * W32[j][i];                                   // 합 = 출력층 바이어스와 가중치(출력층->은익층)
        }
    ...
    for (i = 0; i < InputUnitNo; i++)
    {
        ...
        {
            dW21[j][i] = Eta * d2[j] * O1[p][i] + Alpha * dW21[j][i];   // 역방향 은익층 가중치
            W21[j][i] += dW21[j][i];                                    // W21 가중치
        }
    }
    for (i = 0; i < HiddenUnitNo; i++)
    {
        dbias2[i] = Eta * d2[i] + Alpha * dbias2[i];                    // 역 방향 은익층 노드 활성화
        bias2[i] += dbias2[i];                                          // 은익층 노드 활성화
    }
}
